...
class Trade extends EventEmitter {
    ...
    async execute() {
        ...
        try {
            uuid = await this._privateApi[this._orderMethodName](this.market, quantity, rate);
        } catch (e) {
    ...
    async _closeOrder(uuid) {
        ...
        try {
            ...
            // ORDER_NOT_OPEN means this order is already fulfilled at Bittrex,
            // cancellation was redundant in this case
    ...
    async _adjustQuantity(quantity, orderbookType) {
        ...
        // analyse the **opposite** side of the orderbook and find 85th percentile quantity value
        const buyOrderBook = await this._publicApi.getOrderBook(this.market, 10, orderbookType);
        const percentile85 = stats.percentile(buyOrderBook.map(item => item.Quantity), 0.85);

        // lets not bring too much attention so the price
        // wouldn't start fluctuating rapidly to our disfavour
        return quantity.gt(percentile85) ? percentile85 : quantity;
    }
